...
- [x] Keep `docs/desktop-io-control-roadmap.md` as the lower-level desktop I/O
      proof roadmap.
- [ ] Add a short product-contract section to `docs/project-definition.md`.
...
- [ ] Make each real input action auditable from intent to observed result.
...
- [ ] O1: Turn observe-decide-act-verify into a first-class trace contract.
- [ ] KR1: 100% of real input actions have pre-action and post-action evidence.
...
- [ ] Add `DesktopIoAction` dataclasses and validation.
...
- [ ] Turn a demonstrated routine into editable YAML or website playbook steps.
- [ ] Prefer stable selectors over raw coordinates.
- [ ] Let the operator review and correct generated routines before saving.

### OKRs

- [ ] O1: Ship a Windows recorder that generates valid editable tasks.
...
- [ ] O1: Add routine scheduling and activity pacing.
...
- [ ] A routine can be scheduled for an allowed time window.
...
- [ ] Let the user state a goal and have DeskPilot choose a known routine.
- [ ] Keep deterministic rules authoritative for execution eligibility.
...
- [ ] A user can inspect a failed trace and understand why it failed.

## Phase 9: Recovery And Self-Healing

### Goals

- [ ] Improve routine resilience against ordinary UI variance.
...
- [ ] O1: Add robust recovery planning.
...
- [ ] DeskPilot works with local AI disabled.
